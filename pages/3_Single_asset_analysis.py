...
# 🔥 FIXED: Remove nested @st.cache_data - use single etf_loader cache
def load_and_process_etf(tickers, period):
    """Process data using shared etf_loader cache (no local caching)"""
    try:
        data = load_etfs(tickers, period=period)  # Single cached entrypoint
        processed = {}
        for ticker_name in tickers:
            if ticker_name in data:
                price = data[ticker_name]["prices"].copy()
                if isinstance(price.columns, pd.MultiIndex):
                    price.columns = price.columns.get_level_values(0)
                price = price[["Close", "Adj Close"]].dropna()  # Handle both formats
                if price.empty and "Adj Close" in data[ticker_name]["prices"].columns:
                    price = data[ticker_name]["prices"][["Adj Close"]].dropna()
                if not price.empty:
